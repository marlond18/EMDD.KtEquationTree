...
{
    private static readonly ParserExpr Identifier = Tok(Letter.Then(LetterOrDigit.ManyString(), (h, t) => h + t)).Select<MathStatements.MathStatement>(name => new Identifier(name)).Labelled("identifier");

    private static readonly ParserExpr IdentifierWithSubscript = Tok(TextParser.IdentifierWithSubscript).Labelled("identifier with subscript");
    ...
    private static UnaryFunc CreateCall(IEnumerable<MathStatements.MathStatement> args) => method => Call.Create(method, args.ToArray());
