...
namespace EMDD.KtEquationTree.Exprs.Singles
{
    ...
    public class Literal : Single
    {
        ...
        public override IEnumerable<Expr> Factor() =>
            Value.Factor().ToLookup(f => f).Select(g => (b: g.Key, e: g.Count())).Select(pair => pair.e > 1 ? PowerOp.Create(pair.b, pair.e) : Create(pair.b));

        public override FactorsBase InnerFactor() =>
            Factors.Factors.Create(Value.Factor().ToLookup(f => f).Select(g => (b: g.Key, e: (BigInteger)g.Count())).Select(CreateFactorFromPair));
